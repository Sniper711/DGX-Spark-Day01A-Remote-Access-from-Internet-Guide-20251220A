...
- [9. SSH 超簡單一行指令 登入 DGX Spark Server](#9-ssh-超簡單一行指令-登入-dgx-spark-server)
  - [9.1 從 Client (Mac/PC) 登入 Server (DGX Spark)](#91-從-client-macpc-登入-安裝-server-dgx-spark)
  - [9.2 成功登入後，監控 Server (DGX Spark) 的狀態](#92-成功登入後監控-Server-DGX-Spark-的狀態)
...
SSH 超簡單一行指令 (需輸入 DGX Spark 開機密碼)
```
...
### 9.2 成功登入後，監控 Server (DGX Spark) 的狀態
