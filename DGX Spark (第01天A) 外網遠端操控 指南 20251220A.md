...
- [9. SSH 超簡單一行指令 登入 DGX Spark Server](#9-ssh-超簡單一行指令-登入-dgx-spark-server)
  - [9.1 從 Client (Mac/PC) 登入 Server (DGX Spark)](#91-從-client-macpc-登入-server-dgx-spark)
  - [9.2 兩種範例方法 用 Client (Mac/PC) 控制 Server (DGX Spark)](#92-兩種範例方法-用-client-macpc-控制-server-dgx-spark)
...
## 9. SSH 超簡單一行指令 登入 DGX Spark Server 

### 9.1 從 Client (Mac/PC) 登入 Server (DGX Spark)
SSH 超簡單一行指令 登入 DGX Spark Server (需輸入 DGX Spark 開機密碼)

讓你能用 Client (Mac/PC) 控制 Server (DGX Spark)
```
...
### 9.2 兩種範例方法 用 Client (Mac/PC) 控制 Server (DGX Spark)
方法一，用 Client (Mac/PC) 控制 Server (DGX Spark)，每秒監控一次 Server (DGX Spark) 的 GPU 溫度 (GPU Temp欄位) 、GPU使用率 (GPU-Util欄位)
```
watch -n 1 nvidia-smi
```

方法二，用 Client (Mac/PC) 控制 Server (DGX Spark)，每秒監控一次 Server (DGX Spark) 系統記憶體的 總量 (total欄位)、當前使用量 (used欄位)
