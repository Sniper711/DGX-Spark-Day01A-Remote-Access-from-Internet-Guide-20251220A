...
## 🟩 中文版
> ## 適用情境 與 優點
> **人在外網用 Mac/PC → 透過 WireGuard VPN → SSH 登入家裡的 DGX Spark**
> - **用 WireGuard VPN**
...
- [9. Client (Mac/PC) 登入/控制 Server (DGX Spark)](#9-client-macpc-登入控制-server-dgx-spark)
  - [9.1 SSH 從 Client (Mac/PC) 登入 Server (DGX Spark)](#91-ssh-從-client-macpc-登入-server-dgx-spark)
  - [9.2 兩種範例方法 用 Client (Mac/PC) 控制 Server (DGX Spark)](#92-兩種範例方法-用-client-macpc-控制-server-dgx-spark)
Client (Mac/PC) 登入/控制 Server (DGX Spark) 
---
...
## 9. Client (Mac/PC) 登入/控制 Server (DGX Spark) 

### 9.1 SSH 從 Client (Mac/PC) 登入 Server (DGX Spark)
